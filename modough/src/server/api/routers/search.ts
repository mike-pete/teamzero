...
const getAddress = async (address: string, prisma: PrismaClient) => {
  const addressResult = await prisma.addresses.findFirst({ where: { address: address } })
  return addressResult
}

type GeocodeResult = { type: string, features: { type: string, geometry: { type: string, coordinates: number[] }, bbox: number[], properties: object[] }[] }
